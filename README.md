...
# Schema Theory:
Schema theory is the study of schemas or schematas from the point of view of computer science and discrete mathematics. Schema theory has a lot of applications ranging from Computational Neuroscience (neural encoding...etc) to Bioinformatics (Genome Assembly, Genome Segmentation, Optimal Alignment...etc) to Cognitive Benchmarking, Cognitive Constructivism (Schemata), Comprehensive Input hypothesis (Semantic Selection via Tokenization-Encoding)...etc. Schemas or Schemata are usually dicussed within the real of constructivist learning theories such Jean Piaget's cognitive construcitivism. But for our purpose, they are generally the same entity. However, for the sake and scope of this project, we are going to define them in the discrete-mathematical-computer-science sense.
